                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-content collpase show">
                            <div class="card-body">
                                <form class="form form-horizontal form-bordered" action="{{ route('update_sub_child_categories', $query->id) }}" method="post" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    <div class="form-body">
                                        <h4 class="form-section">Edit Sub Child Category</h4>
                                        <div class="row">
                                            <div class="col-md-12">
                                                    ...
                                                    <select id="child_category" name="child_category" class="form-control select_2">
                                                        @if(!empty($child_categories))
                                                            @foreach($child_categories as $row)
                                                                <option value="{{ $row->id }}" @if(old('child_category') == $row->id) selected @endif @if($query_child_category == $row->id) selected @endif>{{ $row->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
