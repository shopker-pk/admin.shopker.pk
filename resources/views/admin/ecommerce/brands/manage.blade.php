                                ...
                                <form class="form form-horizontal form-bordered">
                                    <div class="form-body">
                                        ...
                                        <div class="row">
                                        	...
                                        	<div class="col-md-1">
                                        		<a href="{{ route('add_brands') }}"><i class="ft-plus"></i> Add</a>
                                    		</div>
                                    		<div class="col-md-1" id="filter_button">
                                    			<a href="javascript::void(0);" id="add_filter"><i class="ft-filter"></i> Filter</a>
                		                        <input type="hidden" id="search_url" value="{{ route('search_brands') }}">
                                            </div>
                                    	</div>
			                                ...
			                                <tbody>
                                                @if(!empty($query))
                                                    @foreach($query as $row)
        			                                    <tr>
        				                                    <td><img src="{{ asset('public/assets/admin/images/ecommerce/brands/'.$row->image) }}" class="m_image" alt="Product Featured Image"></td>
        				                                    <td>{{ $row->name }}</td>
        				                                    <td>
                                                                <label class="switch">
                                                                    <a href="{{ url('/user/admin/ecommerce/brands/ajax/update-status/'. $row->id.'/'. $row->status) }}">
        				                                    ...
        				                                    <td>
        				                                    	<div role="group" class="btn-group">
        														    ...
        														    <div aria-labelledby="btnGroupDrop1" class="dropdown-menu">
        														    	<a href="{{ route('edit_brands', $row->id) }}" class="dropdown-item">Edit</a>
        														    	<a href="{{ route('delete_brands', $row->id) }}" class="dropdown-item">Delete</a>
        														    </div>
        														</div>
        													</td>
        				                               	</tr>
